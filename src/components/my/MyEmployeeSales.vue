...
// 거래처별 총 매출액 계산 함수
const totalSalesByClient = (monthlySales) => monthlySales.reduce((sum, sale) => sum + sale, 0);
...
const calculateCurrentSales = () => {
  const thisMonthTotal = totalSalesByMonth.value[totalSalesByMonth.value.length - 1];
