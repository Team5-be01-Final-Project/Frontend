        ...
        <tbody>
          <tr v-for="(ppc, index) in paginatedPpcs" :key="index">
            ...
            <td class="money-right">
              {{ formatNumberWithCommas(ppc.proUnit) }}
            </td>
            <td class="money-right">
              {{ formatNumberWithCommas(ppc.ppcSale) }}
            </td>
            <td class="index-center">
              <VaButton
                color="warning"
                class="mr-6 mb-2"
                @click="openEditModal(ppc)"
              >
                <va-icon name="edit" />
              </VaButton>
            </td>
...
export default {
  ...
  methods: {
    ...
    deletePpc(deletedPpc) {
      axios
        .delete(`/ppc/${deletedPpc.clientCode}/${deletedPpc.proCode}`)
        .then(() => {
          this.fetchPpcs();
          this.closeModal();
        })
